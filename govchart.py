...
house = '&bill_type=house_resolution'
			...
			for vote in vote_ids:
				...
				try:
					...
					for person_vote in votes:
						...
						#print vote_option
						if vote_option == "+" and pid in senate_members:
							if not pid in people.keys():
								people[pid] = person
							person_bills.append(personBill(pid, bill_id))
				except ValueError:
...
def draw_chart(spectrum, names):
	chart = plt.figure()
	for i in range(len(spectrum)):
		plt.scatter(spectrum[i], i)
		plt.annotate(names[i], (spectrum[i], i))
	plt.show()
...
def generate_vote_spectrum(terms):
	...
	current_people_list = people.keys()
	people_names = [person["lastname"] for person in people.values()]
	print people_names
	#current_people_list = [person["id"] for person in current_people]
	#print "Narrowed down to " + str(len(current_people_list)) + " people"
	m = build_matrix(votes, current_people_list)
	s = matrix_to_spectrum(m)
	print "AAAAAYYYYYY we did it!!"
	print s
	draw_chart(s, people_names)
